...
async function getSchools() {
  try {
    const schools = await executeQuery({
      query: 'SELECT id, name, address, city, state, contact, email_id, image FROM schools ORDER BY name ASC'
    });
    
    const uniqueCities = [...new Set((schools as School[]).map(school => school.city))].sort();
